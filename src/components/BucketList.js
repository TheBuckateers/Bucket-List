...
class BucketList extends React.Component {
  ...
  handleDelete = async (id) => {
    try {
      ...
      let remainingBuckets = this.state.bucketList.filter((item) => item._id !== id);
      this.setState({
            ...
            {this.state.bucketList.map((item) => {
              return <Col xs={12} md={6} lg={4}>
                ...
                >
                  ...
                  <Card.Body>
                    ...
                    <Card.Text className="scroll">
                      <ListGroup className="list-group-flush">
                        ...
                        <ListGroupItem className="border-0 p-0">
                          Population: {item.country.population.toLocaleString()}
                        </ListGroupItem>
                        <ListGroupItem className="border-0 p-0">
                  ...
                  <Card.Footer>
                    <Button variant="danger" size="sm" onClick={() => this.handleDelete(item._id)}>
                      Delete
                    </Button>
                  </Card.Footer>
                </Card>
